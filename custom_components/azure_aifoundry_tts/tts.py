...
async def async_setup_entry(
    ...
    """Set up Azure AI Foundry TTS from a config entry."""
    client = hass.data[DOMAIN][config_entry.entry_id]
    async_add_entities([AzureAIFoundryTTSProvider(config_entry, client)])


class AzureAIFoundryTTSProvider(TextToSpeechEntity):
    """Azure AI Foundry TTS => 'tts.azure_aifoundry_tts_say' in Developer Tools."""

    def __init__(self, config_entry: ConfigEntry, client: GPT4oClient) -> None:
        self._config_entry = config_entry
        self._client = client
        self._name = "Azure AI Foundry TTS"
    ...
    def async_get_supported_voices(self, language: str) -> list[Voice] | None:
        """Return known Azure AI Foundry voices for the voice dropdown."""
        return [Voice(vid, vid.capitalize()) for vid in AZURE_TTS_VOICES]

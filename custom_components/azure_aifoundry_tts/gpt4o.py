"""Client for Azure AI Foundry TTS."""
...
async def _log_api_error(resp: ClientResponse) -> None:
    ...
    _LOGGER.error("Azure AI Foundry TTS API error %s: %s", resp.status, sanitized)


class GPT4oClient:
    """Handles direct calls to Azure AI Foundry for TTS."""
    ...
    def __init__(self, hass, entry):
        ...
        self._endpoint = entry.data.get("endpoint", DEFAULT_ENDPOINT)
        self._deployment_id = entry.data.get("deployment_id", "")

        # Pull default values from options first, then legacy data
        ...
        self._deployment_id = opts.get(
            CONF_DEPLOYMENT_ID, entry.data.get(CONF_DEPLOYMENT_ID, "")
        )
    ...
    async def iter_tts_audio(self, text: str, options: dict | None = None):
        ...
        # Construct the Azure AI Foundry endpoint
        deployment_id = options.get("deployment_id", self._deployment_id) or self._model
        endpoint = options.get("endpoint", self._endpoint) or DEFAULT_ENDPOINT
        tts_endpoint = f"{endpoint.rstrip('/')}/deployments/{deployment_id}/audio/speech"
    ...
    async def get_tts_audio(self, text: str, options: dict | None = None):
        ...
        try:
            ...
            _LOGGER.error(
                "Azure AI Foundry TTS request timed out after %s seconds", REQUEST_TIMEOUT
            )
        except ClientError as err:
            _LOGGER.error("Error generating Azure AI Foundry TTS audio: %s", err)
        except Exception as err:  # pragma: no cover - unexpected errors
            _LOGGER.error("Unexpected error generating Azure AI Foundry TTS audio: %s", err)
        return None, None
